...
show_log_tail() {
  if [[ -f $OMUNTU_INSTALL_LOG_FILE ]]; then
    local log_lines=$((TERM_HEIGHT - LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$OMUNTU_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "Omuntu installation stopped!"
  show_log_tail
  ...
  while true; do
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/omuntu/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$OMUNTU_INSTALL_LOG_FILE"
      else
