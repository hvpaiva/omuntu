...
if [[ -f $OMUNTU_INSTALL_LOG_FILE ]] && grep -q "Total:" "$OMUNTU_INSTALL_LOG_FILE" 2>/dev/null; then
  echo
  TOTAL_TIME=$(tail -n 20 "$OMUNTU_INSTALL_LOG_FILE" | grep "^Total:" | sed 's/^Total:[[:space:]]*//')
...
if sudo test -f /etc/sudoers.d/99-omuntu-installer; then
  sudo rm -f /etc/sudoers.d/99-omuntu-installer &>/dev/null
fi
